...
const isValidDate = (date: Date) => {
  return date <= new Date() && date > new Date("07/01/2019");
};
  ...
  return new Promise((resolve) => {
    ...
    data.forEach((treePurchase) => {
      ...
      if (isValidDate(dayOfPurchase) && isValidTreeQuantity(treePurchase[0])) {
        ...
        aggregationMap[dayOfPurchaseAsDateString] =
          (aggregationMap[dayOfPurchaseAsDateString] || 0) + treePurchase[0];
      }
    });
    resolve(
      Object.entries(aggregationMap).map(([date, numberOfTrees]) => ({
        date: new Date(date),
        numberOfTrees,
      }))
    );
...
const TreeDataContextProvider = ({ children }: { children: JSX.Element }) => {
  ...
  return (
    <TreeDataContext.Provider value={{ loading, data }}>
      {children}
